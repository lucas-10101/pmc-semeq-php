...
function select2AutocompleteClientName() {
  $(".select2-autocomplete-client-name").select2({
    ajax: {
      ...
      processResults: (data) => {
        return {
          results: data.results.map((element, _, __) => ({
            id: element.id,
            text: element.name,
          })),
        };
    ...
    ajax: {
      ...
      processResults: (data) => {
        return {
          results: data.results.map((element, _, __) => ({
            id: element.id,
            text: `${element.name} - R$ ${element.price.toFixed(2)}`,
          })),
        };
      },
    },
...
function select2AutocompleteProductNameSuppliersTableLoader() {
  $(".select2-autocomplete-product-name.list-product-suppliers").on(
    "select2:select",
    function (e) {
      ...
      fetch(
        `/api/suppliers/list_by_product.php?product_id=${selected.id}`
      ).then((response) => {
...
function addSelectedProductToCart() {
  ...
  let selectedProduct = selectElement.options.item(
  ...
  removeBtn.onclick = () =>
    removeProductFromCart(`product-cart-item-${itemNumber}`);
...
function updateTotalFieldOnScren() {
  ...
  itensTable.forEach((element, _, __) => {
    let parts = `${element.innerText}`.split("R$ ");
    let valuePerUnit = parseFloat(parts[1]);
    total += valuePerUnit * element.getAttribute("quantity");
  });
...
function saveSell() {
  ...
  if (!isComplete) {
    callModalBox(
      ...
      "Verifique as informações da venda pois algumas podem estar ausentes"
    );
    return;
  }
