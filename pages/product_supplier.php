...
<div class="container-fluid rounded mx-auto w-75 p-3 bg-body-secondary text-center">
    ...
    <form action="#" method="post" class="row justify-content-around">
        ...
        if ($_POST["acao"] == "salvar") {
            ...
            $toPersist->supplier_id = filter_input(INPUT_POST, "supplier_id", FILTER_SANITIZE_NUMBER_INT);
            ...
            $toSave->product_id = intval(filter_input(INPUT_POST, "product_id", FILTER_SANITIZE_NUMBER_INT));
            $toSave->supplier_id = filter_input(INPUT_POST, "supplier_id", FILTER_SANITIZE_NUMBER_INT);
                ...
                <div class="form-control">
                    <select class="select2 w-100" size="size=" 2" name="product_id" required>
                        ...
                        foreach ($products as $product) {
                            ?>
                            <option value="<?= $product->id ?>" <?= $_GET['product_id'] == $product->id ? 'selected' : '' ?>>
                                <?= $product->name ?>
                            </option>
                            <?php
                        }
